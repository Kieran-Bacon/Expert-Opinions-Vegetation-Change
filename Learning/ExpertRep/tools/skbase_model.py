...
#        try:
        ...
        if train:
            ...
            self.buckets = np.array([[mini+i*bucket_size, mini+(i+1)*bucket_size] for i in range(num_buckets)])
#        except:
#            self.buckets = None
        ...
        return np.argmax(np.logical_and(targets>self.buckets[:,0][None,], targets<self.buckets[:,1][None,]), axis=1)
    ...
    def score(self, test_data: list, test_targets: list):
        ...
        bucketed_targets = self._bucket_targets(targets=test_targets, train=is_regressor(self.model)) #Regressors dont have the bucketing fit already
        bucketed_output = self._bucket_targets(targets=predictions, train=False)
        precision = calculate_precision(bucketed_targets, bucketed_output)
