...
opinions of experts, something that has both potential for abuse
...
clear disclaimers illustrating the nature of the legal requirements; a simple accounts page showing all the information partaining to the user; and a delete account button to
...
to forego this right. This provides us the flexibility to choose the machine learning technique that yields the highest performance \cite{eu:gdpr}. 
...
For example, it may be possible that to learn the patterns in the CMO data, it may require an infeasable number of labeled data points. 
...
\item Webtool skeleton (28/11/2017) - The server for the site is up and running and a database has been created to store the relevant data that will be used in the project. 
...
\item Data collection functionality (15/1/2018) - The page where an expert gives their opinion is fully functional. This includes visual representation of the models, user input, and data capture. Completion of this milestone is instrumental in the development of the rest of the project.

\item Basic expert model (30/1/2018) - Once the data collection is implemented we can begin to develop our expert model. Analysis into the machine learning technique will begin. A prototype expert model will be used to test other components.

\item Chosen set of algorithms (10/2/2018) - After a expert model has been created we can start experimenting with the different types of machine learning algorithms. Through this experimentation we will come up with a single/small set of machine learning algorithms that users can choose from.

\item Training stage complete (17/2/2018) - At this point an expert should be able to login to the site, give their opinions on CMOs, have those opinions trained into a expert model, and use that model to provide opinions on new data. This element can begin development once the data collection and basic learner are completed.
